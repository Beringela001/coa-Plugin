...
<header class="ps-coa-report-hero">
	...
	<div class="ps-coa-report-hero__outcome ps-coa-report-hero__outcome--<?php echo esc_attr( $test['public_status_tone'] ); ?>">
		...
		<?php if ( $test['public_notes'] || $test['report_notes'] ) : ?><div class="ps-coa-outcome-notes"><?php echo wp_kses_post( wpautop( implode( "\n\n", array_unique( array_filter( array_map( 'trim', array( $test['public_notes'], $test['report_notes'] ) ) ) ) ) ) ); ?></div><?php else : ?>
		...
		<?php endif; ?>
		<?php if ( 'failed' === $test['coa_status'] && $test['release_decision_note'] ) : ?><p class="ps-coa-release-decision"><strong><?php esc_html_e( 'Release decision:', 'pepselect-coa-archive' ); ?></strong> <?php echo esc_html( $test['release_decision_note'] ); ?></p><?php endif; ?>
