...
final class COA_Test_Admin {
	...
	public function render_column( $column, $post_id ) {
		...
		elseif ( 'coa_status' === $column ) { $statuses = COA_Test_Fields::statuses(); echo isset( $statuses[ $value ] ) ? '<span class="ps-coa-status">' . esc_html( $statuses[ $value ] ) . '</span>' : '&mdash;'; }
	...
	public function sortable_columns( $columns ) { foreach ( array( 'test_date', 'batch_number', 'compound_id', 'purity_percentage', 'coa_status', 'is_current' ) as $key ) { $columns[ $key ] = $key; } return $columns; }
	...
	public function apply_query_controls( $query ) {
		...
		if ( in_array( $orderby, array( 'test_date', 'batch_number', 'compound_id', 'purity_percentage', 'coa_status', 'is_current' ), true ) ) { $query->set( 'meta_key', $orderby ); $query->set( 'orderby', in_array( $orderby, $numeric, true ) ? 'meta_value_num' : 'meta_value' ); }
