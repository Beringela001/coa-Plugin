...
final class Archive_Cache {
	...
	public static function get( $search, $page, $per_page, $eligible_ids ) {
		$found = false;
		$value = wp_cache_get( self::key( $search, $page, $per_page, $eligible_ids ), self::GROUP, false, $found );
		return $found && is_array( $value ) ? $value : null;
	}

	/** Stores one archive result in the plugin-only cache group. @return void */
	public static function set( $search, $page, $per_page, $eligible_ids, $value ) {
		wp_cache_set( self::key( $search, $page, $per_page, $eligible_ids ), $value, self::GROUP, HOUR_IN_SECONDS );
	}

	/** Builds a stable key that separates unsearched and normalized searched archives. @return string */
	public static function key( $search, $page, $per_page, $eligible_ids ) {
		...
		return sprintf( 'archive-v%d-%s-p%d-n%d-%s', self::version(), $scope, max( 1, absint( $page ) ), max( 1, absint( $per_page ) ), md5( implode( ',', $ids ) ) );
	}
