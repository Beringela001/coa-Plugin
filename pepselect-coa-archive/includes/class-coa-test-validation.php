...
final class COA_Test_Validation {
	...
	public static function sanitize( $value, $key = '' ) {
		...
		if ( 'lab_verification_url' === $key ) { return esc_url_raw( trim( (string) $value ), array( 'http', 'https' ) ); }
	...
	public function validate( $valid, $value, $field, $input ) {
		...
		if ( 'lab_verification_url' === $name && '' !== $raw && ! wp_http_validate_url( $raw ) ) { return __( 'Enter a valid HTTP or HTTPS URL.', 'pepselect-coa-archive' ); }
