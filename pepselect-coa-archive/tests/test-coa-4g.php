...
class PepSelect_COA_Archive_COA_4G_Test extends WP_UnitTestCase {
	...
	public function test_archive_catalog_template_renders_required_scoped_sections_and_preserves_card_structure() {
		...
		foreach ( array( 'ps-coa-archive--catalog-layout', 'partials/archive-hero.php', 'The selection,', 'on the record.', 'Showing %1$s of %2$s compounds', 'No matching compounds', 'Clear search' ) as $needle ) { $this->assertStringContainsString( $needle, $template ); }
		foreach ( array( 'Every batch has a', 'permanent address.', 'Independent labs', 'Batch-level COAs', 'Published unedited', 'role="search"', 'name="coa_search"', 'esc_attr( $search )' ) as $needle ) { $this->assertStringContainsString( $needle, $hero ); }
	...
	public function test_archive_card_uses_compound_stock_image_without_batch_or_incoming_leaks() {
		...
		$model = $this->view->archive_compound( get_post( $compound ), array( get_post( $incoming ), get_post( $current ) ) ); $this->assertSame( 0, $model['compound_image_id'] ); $this->assertSame( 'local-placeholder', $model['archive_image_source'] ); $this->assertNotContains( $model['compound_image_id'], array( $incoming_image, $other_image ) );
		delete_post_meta( $current, 'batch_vial_photo' ); update_post_meta( $current, '_thumbnail_id', $featured_image ); $model = $this->view->archive_compound( get_post( $compound ), array( get_post( $incoming ), get_post( $current ) ) ); $this->assertSame( 0, $model['compound_image_id'] ); $this->assertSame( 'local-placeholder', $model['archive_image_source'] );
	...
	private function image( $name ) { $id = self::factory()->post->create( array( 'post_type' => 'attachment', 'post_status' => 'inherit', 'post_mime_type' => 'image/jpeg', 'guid' => 'https://example.org/' . $name ) ); update_post_meta( $id, '_wp_attached_file', '2026/09/' . $name ); return $id; }
}
