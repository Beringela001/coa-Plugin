...
class PepSelect_COA_Archive_COA_4E_1_Test extends WP_UnitTestCase {
	...
	public function test_01_bar_renders_for_approved_complete_report() { $model = $this->report( $this->fixture() ); $this->assertTrue( $model['show_qc_strip'] ); $this->assertFalse( $model['is_full_qc_documented'] ); $this->assertNotEmpty( $model['qc_strip_rows'] ); $this->assertStringContainsString( "\$test['show_qc_strip']", $this->template( 'single-coa-report.php' ) ); }
	...
	public function test_07_net_content_renders_when_saved() { $row = $this->row( $this->report( $this->fixture() ), 'net-content' ); $this->assertSame( 'Measured content', $row['short_label'] ); $this->assertSame( '31.01 mg avg', $row['detail'] ); $this->assertFalse( $row['status']['success'] ); }
	...
	public function test_11_fentanyl_screen_renders_when_saved() { $row = $this->row( $this->report( $this->fixture() ), 'fentanyl' ); $this->assertSame( 'Fentanyl screening', $row['short_label'] ); $this->assertSame( 'Not detected', $row['detail'] ); }
	...
	public function test_16_category_count_matches_rendered_categories() { $test = $this->report( $this->fixture() ); $this->assertSame( count( $test['qc_strip_rows'] ), $test['qc_category_count'] ); $this->assertSame( 7, $test['qc_category_count'] ); ob_start(); include dirname( __DIR__ ) . '/templates/partials/full-qc-status-strip.php'; $html = ob_get_clean(); $this->assertSame( 7, substr_count( $html, '<li ' ) ); }
