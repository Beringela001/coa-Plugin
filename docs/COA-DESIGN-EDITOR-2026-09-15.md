...
Live deployed September 15, 2026. After explicit approval, only the September 12, 8:31 AM backup “Before Reddit test reference 0.3.1 LIVE” was deleted; four newer backups were retained. Fresh manual backup “Before COA visual editor 0.7.16 LIVE - Sep15” completed at 9:50 PM with Restore to available. The verified package above replaced 0.7.15; WordPress confirmed success and the active plugin row shows 0.7.16. Kinsta Clear all caches was requested; normal public URLs served the updated templates.

Live checks passed: editor loads actual public records and retained all sampled custom waiting/submitted/in-testing/full-QC copy; no Live settings were saved. Reta 10 vial image loaded (768px source), its exact customer note remains in the separate callout, and the hero has no redundant outcome pill. Mobile editor preview shows equal image/status cards with vial left. Latest KPV retains its saved fentanyl evidence and omits explicitly untested heavy-metals/endotoxin rows. Failed Reta 20 retains its rejection note and Testing failed heading. Pending Reta 10 publicly uses the saved “We have more arriving.” heading and matching description. Anonymous design API returns HTTP 401/rest_forbidden. Live editor left ready for use. No report data, PDFs, inventory or Ops application changed.
